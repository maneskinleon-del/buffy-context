...
**Requisito:** `leak ≤ 0.308` sobre el benchmark congelado.
...
**Requisito:** `attr ≥ 13/20` sobre el benchmark congelado.
...
**Requisito:** `pRel ≥ 0.121` (umbral del contrato) — objetivo operativo `≥ 0.500`.

- **Justificación:** 0.121 es el piso del contrato; 0.500 es el objetivo realista
  dado que T alcanzó 0.630.
...
**Requisito:** `contain ≥ 0.80` sobre el benchmark congelado.
...
**Requisito:** para cada Q ∈ {Q01…Q10}: `attr_Q(T) ≥ attr_Q(A)` (control baseline).
...
**Requisito:** `attr_Q05 ≥ 1` — el pasaje con `adb devices -l` debe entrar al
...
**Requisito:** 2 corridas de la misma config producen JSONs idénticos per-query
(`determinism_hash` idéntico).

- **Justificación:** contrato de la serie; base de la reproducibilidad.
- **Evidencia:** 17E: r1=r2 en las 4 configs (8 corridas).
- **Estado:** ✅ CUMPLIDO.

### R8 — Exclusión de noise de sesión (mecanismo V1)

**Requisito:** los pasajes `is_session_noise` que no son gold NO entran al
...
**Requisito:** la expansión de query debe incluir vocabulario técnico de dominio
...
**Requisito:** `tokens_avg ≤ BUDGET_TOKENS` (10400) por query — objetivo operativo
`tokens_avg ≤ 600`.

- **Justificación:** el contexto final debe caber en el presupuesto; T usa ~405
  tokens promedio (17E), muy por debajo del límite.
...
| Requisito | Umbral | 17E-T | Estado |
|---|---|---|---|
| R1 leak | ≤ 0.308 | 0.307 | ✅ |
| R2 attr | ≥ 13/20 | 13/20 | ✅ |
| R3 pRel | ≥ 0.121 (obj. 0.500) | 0.630 | ✅ |
| R4 contain | ≥ 0.80 | 1.0 | ✅ |
...
| R10 tokens | ≤ 10400 (obj. 600) | 404.9 | ✅ |

**Veredicto: 10/10 requisitos cumplidos por T = V1 + DICT_H1_B sobre el fixture
congelado.** T es CANDIDATO A ADOPCIÓN (decisión final del usuario).
...
1. **Decisión de adopción del usuario**: 17E = PASS / CANDIDATO A ADOPCIÓN. Si se
   ...
   (runtime real, no fixture) con su propio gate de regresión.
