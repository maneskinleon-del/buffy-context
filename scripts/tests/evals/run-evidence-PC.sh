#!/usr/bin/env bash
# run-evidence-PC.sh — Paso 12: experimento de evidence-aware passage selection
# (variantes E1=S3-heading, E2=S4-semántico, E3=S4+0.5·S3) sobre el EVAL PC.
#
# Responde: ¿una señal basada en el CONTENIDO del pasaje puede distinguir un
# pasaje que realmente responde a la consulta de otro que solo comparte
# vocabulario/contexto? (Q1/Q2 demostraron que densidad/estructura NO alcanzan;
# el contenido es la capa pendiente.)
#
# Evidencia que motiva el diseño (medida HOY sobre el contexto R1, 60 pasajes,
# 17 con aguja):
#   - S1 n-gram contiguo (query∪X) 1.0× y S2 cmd-lines 1.0× → DESCARTADAS.
#   - S3 heading de sección: 17.7× (prec 1.0 @ rec 0.35) — barata, bajo recall.
#   - S4 bge-m3 cosine(pasaje, query+X): prec 0.867 @ rec 0.765 @ θ=0.55
#     (13 TP / 2 FP en 43 ruidosos) — la mejor discriminación de la serie.
#   - Tres conceptos en R1: found 20/20 · selected 14/20 · attributed 14/20;
#     solo 6/20 en la capa de pasajes → la señal se evalúa como selector de ESA
#     capa (pRel mide solo pasajes).
#
...
# Cambia UNA cosa: la capa de SELECCIÓN por contenido entre el ranking y el
# contexto. bge-m3 NO genera candidatos ni participa del ranking (falló en D y
# R2): solo puntúa el CONTENIDO del pasaje contra la query expandida.
#
#   E1-S3          gate por heading: descartar pasajes cuya sección markdown no
#                  comparte ≥1 término significativo (>3 chars) con query ∪ X.
#   E2-S4          gate semántico: bge-m3 cosine(pasaje, query expandida) ≥ θ;
#                  θ = 0.55 fijado a priori (diagnóstico del Paso 12).
#   E3-S3+S4       score = S4 + 0.5·S3 ≥ θ (la ponderación medida hoy; NO 0.5/0.5
#                  — esa escala con θ=0.55 rechazaría casi todo, bug corregido
#                  en review).
#
...
#
# Uso:
#   run-evidence-PC.sh [--ev e1|e2|e3] [--expand none|f1|f2] [--model bge-m3]
#                      [--ollama URL] [--out FILE] [--limit N] [--reindex]
#                      [--json] [--quiet]
#   --ev      e1|e2|e3   variante de contenido (default: e2). CADA variante es un
#                        resultado experimental independiente (mismo gate).
#   --expand  none|f1|f2 Paso 13: passage candidate expansion (rama P) ANTES del
#                        scorer E2. f1 = pasajes de router_knowledge · f2 = f1 +
#                        top-10 archivos del pool por R1. Congelados: H2/R1/E2/θ/
#                        presupuesto/gate. gold_files NUNCA es señal de expansión.
#   --out     archivo de salida (default: baseline-E1|E2|E3-evidence-…-2026-08-12.json
#                        o baseline-F1|F2-expansion-PC-2026-08-13.json)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --ev) EV_MODE="${2:?falta e1|e2|e3}"; shift 2 ;;
    --expand) EXPAND="${2:?falta none|f1|f2}"; shift 2 ;;
    --selector) SELECTOR="${2:?falta bge|phi}"; shift 2 ;;
    --dump-pool) DUMP_POOL="${2:?falta dir}"; shift 2 ;;
...
case "$EV_MODE" in
  e1|e2|e3) ;;
  *) echo "variante inválida: $EV_MODE (e1|e2|e3)" >&2; exit 2 ;;
esac

case "$EXPAND" in
  none|f1|f2) ;;
  *) echo "expansión inválida: $EXPAND (none|f1|f2)" >&2; exit 2 ;;
esac

case "$SELECTOR" in
  bge|phi) ;;
  *) echo "selector inválido: $SELECTOR (bge|phi)" >&2; exit 2 ;;
esac
...
if [ -z "$OUT_FILE" ]; then
  LABEL="E1"; [ "$EV_MODE" = "e2" ] && LABEL="E2"; [ "$EV_MODE" = "e3" ] && LABEL="E3"
  DATE="2026-08-12"
  if [ "$EXPAND" = "f1" ]; then LABEL="F1"; DATE="2026-08-13"; fi
  if [ "$EXPAND" = "f2" ]; then LABEL="F2"; DATE="2026-08-13"; fi
  OUT_FILE="$SCRIPT_DIR/baseline-$LABEL-expansion-PC-$DATE.json"
fi
...
RESULT_FILE="$(EV_MODE="$EV_MODE" MODEL="$MODEL" OLLAMA_URL="$OLLAMA_URL" LIMIT="$LIMIT" REPO_DIR="$REPO_DIR" EVAL="$EVAL" G1_REF="$G1_REF" R1_REF="$R1_REF" TMP="$TMP" EVAL_HASH="$EVAL_HASH" CACHE_DIR="$CACHE_DIR" REINDEX="$REINDEX" QUIET="$QUIET" EXPAND="$EXPAND" SELECTOR="$SELECTOR" DUMP_POOL="$DUMP_POOL" python3 - "$REPO_DIR" "$EVAL" "$G1_REF" "$R1_REF" "$TMP" "$EV_MODE" "$MODEL" "$OLLAMA_URL" "$LIMIT" "$EVAL_HASH" "$CACHE_DIR" "$REINDEX" "$EXPAND" "$SELECTOR" "$DUMP_POOL" <<'PY'
import json, os, re, subprocess, sys, time, urllib.request, urllib.error, hashlib, math, unicodedata, array

repo, eval_path, g1_ref, r1_ref, tmpdir, ev_mode, model, ollama_url, limit, eval_hash, cache_dir, reindex, expand, selector, dump_pool = (
    sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7],
    sys.argv[8], int(sys.argv[9]), sys.argv[10], sys.argv[11], sys.argv[12] == "true", sys.argv[13],
    sys.argv[14], sys.argv[15],
)
EXPAND = expand            # none | f1 | f2 (Paso 13: passage candidate expansion)
SELECTOR = selector        # bge | phi (Paso 14B: función de decisión del gate)
DUMP_POOL = dump_pool      # dir para snapshot congelado del pool (Paso 14A)
P_EXPAND_TOP_K = 10        # F2: top-K archivos del pool fuera de kno (a priori, = LIMIT)
...
THETA_S4 = 0.550   # E2/E3: umbral de cosine semántico fijado a priori (diagnóstico Paso 12: 13 TP/2 FP @ 0.55)
W_S3 = 0.5         # E3: peso del heading sobre el score semántico (score = S4 + 0.5·S3, la ponderación medida)
...
def tile_windows(path, pad=PAS_PAD):
    """Paso 13 (rama P): ventanas no-solapadas de 2·pad+1 líneas por archivo —
    la descomposición de "todos los pasajes" consistente con G1-VENTANA ±pad."""
    total = len(file_lines(path))
    if total <= 0:
        return []
    win = 2 * pad + 1
...
# ── PASO 12: señales de CONTENIDO (E1/E2/E3) ──
_HEAD_RE = re.compile(r'^#{1,6} ')
def heading_of_passage(path, s, e):
    lines = file_lines(path)
    h = ''
    for i in range(min(s - 1, len(lines))):
        ln = lines[i]
        if _HEAD_RE.match(ln):
            h = ln
    return re.sub(r'\s+', ' ', deaccent(h).lower()).strip()

def s3_heading_score(path, s, e, phrases):
    nh = heading_of_passage(path, s, e)
    if not nh:
        return 0.0
    uniq = set()
    for ph in phrases:
    ...
    hits = 0
    for t in uniq:
        if re.search(r'(?<![a-z0-9_])' + re.escape(t) + r'(?![a-z0-9_])', nh):
            hits += 1
...
def passage_emb_path(key_text, dim):
...
def embed_all(texts, batch=10):
    """embeddings por lotes con CACHE de pasajes (infra, no variable del experimento).

    clave = sha1 del texto truncado a 2000 chars (exactamente lo que ve el
    modelo) → cache_dir/passages/<sha1>-<dim>.emb (float64). Se almacena float64
    —NO float32— porque el JSON de Ollama emite la precisión doble exacta del
    vector y float32 perdería ~4e-10 (0/1024 valores idénticos en el test), lo
    que rompería la equivalencia bit-a-bit y podría voltear un pasaje borderline
    en el gate. Con float64 el round-trip es exacto (1024/1024) y bge-m3 es
    determinista entre llamadas (verificado) → el vector cacheado ES el mismo
    que el computado sin cache. El hash ES el contenido: no hay invalidación.
    """
    out = [None] * len(texts)
    todo = [(i, t[:2000]) for i, t in enumerate(texts) if t.strip()]
    missing = []
    for j, key_text in todo:
        path = passage_emb_path(key_text, dim)
        try:
            data = array.array('d')
    ...
    for k in range(0, len(missing), batch):
        ...
        embs = ollama_embed_batch([kt for _j, kt, _p in chunk])
...
def phi_judge(qtext, ptext):
    """Paso 14B: juez generativo Phi (decisión gold/no-gold). temp=0 + seed fijo
    para determinismo; parsea la respuesta con deaccent (SÍ → SI).
    Mecanismo DISTINTO del scorer bge-m3 (razonamiento semántico, no cosine)."""
    prompt = ("Consulta: %s\n\nPasaje:\n%s\n\n¿El pasaje contiene la evidencia "
              "que responde la consulta? Responde solo SI o NO."
              % (qtext, ptext[:1500]))
    try:
        data = ollama_post('/api/generate', {"model": "phi3.5", "prompt": prompt,
                                             "stream": False, "temperature": 0,
                                             "seed": 42, "options": {"num_predict": 8}})
        ans = deaccent((data.get("response") or "").strip().upper())
        return 1.0 if ans.startswith("SI") else 0.0
    except Exception:
        return 0.0
...
pool_snap = {"eval_hash": eval_hash, "expand": EXPAND, "selector": SELECTOR,
             "generated": time.strftime("%Y-%m-%dT%H:%M:%S"), "queries": []}
...
for q in fixture["queries"]:
    ...
    # ── PASO 13: PASSAGE CANDIDATE EXPANSION (LA VARIABLE) ──
    # Rama P: pasajes candidatos por ARCHIVO (tile ±4 no-solapado de 2·PAS_PAD+1
    # líneas), etiquetados {'P'}. F1 = archivos de router_knowledge; F2 = F1 +
    # top-K archivos del pool por orden R1 (fuera de kno). El pool expandido
    # (L∪X∪S∪P) se re-ordena con la MISMA función R1-LEX congelada (mismas
    # señales/pesos) — el SET crece, el procedimiento no cambia. gold_files
    # NUNCA es señal de expansión (regla de la serie).
    p_expand_files = []
    p_pool_items = []
    if EXPAND != "none":
        kno_set = set(kno)
        ...
        if EXPAND == "f2":
            ...
            for it in ordered:
                ...
                seen_f.add(it[0])
                files.append(it[0])
                if len(files) - len(kno) >= P_EXPAND_TOP_K:
                    break
        p_wins = {}
        for f in files:
            if not os.path.isfile(os.path.join(repo, f)):
                continue
            for (s0, e0) in tile_windows(f):
                txt = passage_text(f, s0, e0)
                if not txt.strip():
                    continue
                p_pool_items.append((f, s0, txt, None, {'P'}, 0.0))
                p_wins[(f, s0)] = (s0, e0)
                p_expand_files.append(f)
        if p_pool_items:
            # item_pass para ventanas P (nunca pisa ventanas de ítems L/X/S
            # existentes; la colisión (path, inicio) es determinista y rarísima)
            for k, v in p_wins.items():
                if k not in item_pass:
                    item_pass[k] = v
            pool_items = pool_items + p_pool_items
            ordered = [it for _, it in sorted(
                [(score_r1(item_signals(it, item_pass, q_tokens, x_tokens)), it) for it in pool_items],
                key=lambda x: (-x[0], x[1][0], x[1][1]))]
    p_files_uniq = sorted(set(p_expand_files))

    if DUMP_POOL:
        qsnap = {"qid": qid, "query": query, "terms": terms,
                 "gold_facts": [f["text"] for f in gold_facts if f["text"].strip()],
                 "gold_files": sorted(gold_files), "kno": sorted(kno),
                 "passages": []}
        seen_pg = set()
        for it in pool_items:
            s0, e0 = item_pass[(it[0], it[1])]
            key = (it[0], s0, e0)
            if key in seen_pg:
                continue
            seen_pg.add(key)
            txt = passage_text(it[0], s0, e0)
            label = 0
            for f in gold_facts:
                if f["text"].strip() and deaccent(f["text"].lower()) in deaccent(txt).lower():
                    label = 1
                    break
            qsnap["passages"].append({"path": it[0], "s": s0, "e": e0,
                                      "ramas": sorted(it[4]), "label": label,
                                      "text": txt})
        pool_snap["queries"].append(qsnap)

    # ── EVIDENCE-AWARE SELECTION (LA VARIABLE del Paso 12) ──
    # Recorrer el pool ordenado por R1; seleccionar ítems cuyo pasaje supera el
    # gate de CONTENIDO: E1-S3 (heading ≥ 1 término significativo), E2-S4
    # (bge-m3 cosine(pasaje, query expandida) ≥ THETA_S4), E3 (S4 + 0.5·S3 ≥
    # THETA_S4). SIN dedup estructural (fue la variable Q1/Q2 del Paso 11,
    # descartada). Los seleccionados entran al contexto en orden R1.
    phrases = [query] + terms
    if ev_mode in ("e2", "e3") and SELECTOR == "bge":
        q_emb = norm(ollama_embed_batch([" ".join(phrases)])[0])
        uniq_pass = {}
        for it in pool_items:
            s0, e0 = item_pass[(it[0], it[1])]
            uniq_pass.setdefault((it[0], s0, e0), it)
        keys = list(uniq_pass.keys())
        embs = embed_all([passage_text(p, s, e) for (p, s, e) in keys])
    ...
    for item in ordered:
        ...
        if ev_mode == "e1":
            score = s3_heading_score(item[0], s, e, phrases)
            keep = score > 0.0
        elif SELECTOR == "phi":
            keep = phi_judge(" ".join(phrases), passage_text(item[0], s, e)) > 0.0
        else:
            pt = passage_text(item[0], s, e)
            score = cosine_sim(emb_map[(item[0], s, e)], q_emb)
            if ev_mode == "e3":
                score += W_S3 * s3_heading_score(item[0], s, e, phrases)
            keep = score >= THETA_S4
        if not keep:
            dropped_content += 1
            continue
    ...
    # ── tres conceptos por query (found / selected / attributed) ──
    ctx_full = {}
    for f in kno:
        ctx_full[f] = " ".join(file_lines(f))
    for (p, (s0, e0)) in ctx_ranges_q:
        ctx_full[p] = ctx_full.get(p, "") + " " + passage_text(p, s0, e0)
    pool_passages = set()
    for it in pool_items:
        s0, e0 = item_pass[(it[0], it[1])]
        pool_passages.add((it[0], s0, e0))
    pool_text = " ".join(deaccent(passage_text(p, s0, e0)).lower() for (p, s0, e0) in pool_passages)
    ev_p_pool_text = " ".join(
        deaccent(passage_text(it[0], s0, e0)).lower()
        for it in p_pool_items
        for (s0, e0) in [item_pass[(it[0], it[1])]]
    )
    ev_found = ev_selected = ev_attributed = ev_passage_available = 0
    for f in gold_facts:
        if not f["text"].strip():
            continue
        nd = deaccent(f["text"].lower())
        if nd in pool_text:
            ev_found += 1
        if nd in ev_p_pool_text:
            ev_passage_available += 1
        if any(nd in deaccent(t).lower() for t in ctx_full.values()):
            ev_selected += 1
        if any(p in gold_files and nd in deaccent(t).lower() for p, t in ctx_full.items()):
            ev_attributed += 1
    ...
    es = {
        ...
        "evidence_selected": ev_selected,
        "evidence_attributed": ev_attributed,
    }
    if EXPAND != "none":
        es.update({
            "p_expand_files": len(p_files_uniq),
            "p_pool_passages": len(p_pool_items),
            "passage_available": ev_passage_available,
        })
    ev_stats.append(es)

    per_query.append({
        "id": qid, "query": query, "coverage": q.get("coverage"),
        "strategy": "evidence", "evidence_mode": ev_mode,
        ...
        "evidence_stats": ev_stats[-1],
    })
    if EXPAND != "none":
        per_query[-1]["g_h0"]["pool_stats"]["P"] = len(p_pool_items)

if DUMP_POOL:
    os.makedirs(DUMP_POOL, exist_ok=True)
    snap_path = os.path.join(DUMP_POOL, "selector-pool-frozen-2026-08-13.json")
    json.dump(pool_snap, open(snap_path, "w", encoding="utf-8"), indent=2, ensure_ascii=False)
    sys.stderr.write("pool snapshot (14A): %s\n" % snap_path)
...
label = {"e1": "E1", "e2": "E2", "e3": "E3"}[ev_mode]
variant_name = {"e1": "CONTENT heading (S3: sección markdown ≥ 1 término significativo)",
                "e2": "CONTENT semántico (S4: bge-m3 cosine(pasaje, query expandida) ≥ 0.55)",
                "e3": "CONTENT semántico+heading (S4 + 0.5·S3 ≥ 0.55)"}[ev_mode]
if EXPAND == "f1":
    label = "F1"
    variant_name += " + EXPANSIÓN F1 (rama P: pasajes de router_knowledge)"
elif EXPAND == "f2":
    label = "F2"
    variant_name += " + EXPANSIÓN F2 (rama P: router + top-%d del pool)" % P_EXPAND_TOP_K
if SELECTOR == "phi":
    label += "-PHI"
    variant_name += " + SELECTOR Phi (juez generativo temp=0, seed=42)"
pipeline_desc = (
    "pool CONGELADO de H2 (L ∪ X(h2) ∪ S, dict_hash %s) → orden CONGELADO R1-LEX "
    "→ %s → pasajes VENTANA ±%d → presupuesto %d tokens → ctx"
) % (dict_hash(), variant_name, PAS_PAD, BUDGET_TOKENS)

# agregados de evidencia
agujas_tot = sum(agujas_r1_total); agujas_pres = sum(agujas_r1_preservadas)
fracs = [s["fraccion_podada"] for s in ev_stats if s["fraccion_podada"] is not None]
toks_ah = [s["tokens_ahorrados"] for s in ev_stats if s["tokens_ahorrados"] is not None]
tok_ev = [s["tokens_evidencia_absolutos"] for s in ev_stats]
ev_found_tot = sum(s["evidence_found"] for s in ev_stats)
ev_selected_tot = sum(s["evidence_selected"] for s in ev_stats)
ev_attributed_tot = sum(s["evidence_attributed"] for s in ev_stats)
ev_avail_tot = sum(s.get("passage_available", 0) for s in ev_stats)
...
summary = {
    ...
    "ev_params": {"theta_s4": THETA_S4, "w_s3": W_S3, "ev_mode": ev_mode, "ranking": "r1"},
    ...
    "aggregate": {
        ...
        "evidence": {
            ...
            "dropped_content_total": sum(s["dropped_content"] for s in ev_stats),
            "evidence_found_total": ev_found_tot,
            "evidence_selected_total": ev_selected_tot,
            "evidence_attributed_total": ev_attributed_tot,
        },
    },
    "per_query": per_query,
}
...
if EXPAND != "none":
    summary["ev_params"]["expansion"] = EXPAND
    summary["ev_params"]["p_expand_top_k"] = P_EXPAND_TOP_K
    summary["aggregate"]["evidence"]["passage_available_total"] = ev_avail_tot
if SELECTOR == "phi":
    summary["ev_params"]["selector"] = "phi"
    summary["ev_params"]["selector_model"] = "phi3.5"
    summary["ev_params"]["selector_temp"] = 0.0
    summary["ev_params"]["selector_seed"] = 42
...
print("── Variante %s — ev %s · perfil PC · pasajes VENTANA · instrumento v3.1 ──" % (label, d.get("evidence_mode", "?")))
...
print("  determinism_hash: %s (gate G2) · ev_params: %s" % (d.get("determinism_hash"), d.get("ev_params")))
print("  ── agregado ──")
...
print("  ★ evidence: agujas_preservadas=%s (%s/%s, por-gold-fact) · fraccion_podada_avg=%s · tokens_ahorrados_avg=%s" % (
    ap.get("fraction"), ap.get("preserved"), ap.get("total"),
    po.get("fraccion_podada_avg"), po.get("tokens_ahorrados_avg")))
print("           tokens_evidencia_absolutos_avg=%s · dropped_content=%s · found=%s selected=%s attributed=%s" % (
    po.get("tokens_evidencia_absolutos_avg"), po.get("dropped_content_total"),
    po.get("evidence_found_total"), po.get("evidence_selected_total"), po.get("evidence_attributed_total")))
print("  ── por query ──")
...
print("  ── evidence por query (agujas R1 → preservadas · tres conceptos · tokens evidencia) ──")
for q in d["per_query"]:
    ps = q["evidence_stats"]
    print("  %-4s agujas_r1=%d preservadas=%d · found=%d selected=%d attributed=%d · ctx %d/%d pasajes (%.0f%% poda) · tok_evidencia=%d" % (
        q["id"], ps["agujas_r1_total"], ps["agujas_preservadas"], ps["evidence_found"],
        ps["evidence_selected"], ps["evidence_attributed"], ps["n_ctx_passages_q"],
        ps["n_ctx_passages_r1"], 100 * (ps["fraccion_podada"] or 0), ps["tokens_evidencia_absolutos"]))
if d["per_query"] and "passage_available" in d["per_query"][0]["evidence_stats"]:
    print("  ── funnel P por query (rama P: archivos · pasajes · available → selected → attributed) ──")
    for q in d["per_query"]:
        ps = q["evidence_stats"]
        print("  %-4s p_files=%d p_pasajes=%d · found=%d available=%d selected=%d attributed=%d" % (
            q["id"], ps.get("p_expand_files", 0), ps.get("p_pool_passages", 0),
            ps["evidence_found"], ps.get("passage_available", 0),
            ps["evidence_selected"], ps["evidence_attributed"]))
...
base_map = {"A": None, "B": None, "C": None, "D": None, "E": None, "F": None, "G1": None, "G2": None, "H1": None, "H2": None, "R1": None, "R2": None, "F1": None, "F2": None}
base_inst = {}
def _inst_rank(v):
...
if present:
    ...
    for k in ("A", "B", "C", "D", "E", "F", "G1", "G2", "H1", "H2", "R1", "R2", "F1", "F2"):
        bd = base_map[k]
        if not bd:
            continue
        ba = bd["aggregate"]
