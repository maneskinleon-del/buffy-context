#!/usr/bin/env bash
# run-granularity-PC.sh — Paso 16B: granularidad del pasaje (pool REGENERADO)
# ─────────────────────────────────────────────────────────────────────────────
# Pregunta: con la generación del pool REGENERADA por PAS_PAD ∈ {0,1,2,4} sobre
# el corpus real (no el fixture congelado), ¿la granularidad del pasaje rescata
# los golds de comando/símbolo (Q01/Q03/Q05) con H1 real?
#
# Diferencia con 16A (descartado): 16B NO usa selector-pool-frozen-2026-08-13.json
# (que fija la granularidad en ±4). Regenera el pool con el MISMO mecanismo del
# Paso 13 (rama P, EXPAND=f2): L ∪ X ∪ S ∪ P, dedup por (path, s, e).
#
# Reglas de la serie (granularity-DESIGN.md §3-§7 + decisión del usuario 2026-08-14):
#   - query del S1 = query natural + términos H1 REALES (expand_query.py DICT_H1).
#     NUNCA los terms del fixture (contienen H2/oráculo — prohibido).
#   - NUNCA se usa gold_files/gold_facts como señal de generación ni de ranking,
#     y NUNCA se inyecta un pasaje gold al pool para hacerlo seleccionable.
#     Gold files/facts se usan SOLO después, para medir.
#   - F2 conserva su mecánica: router knowledge + top-K archivos candidatos,
#     tiles no solapados de 2·PAS_PAD+1, guard max-passages=400.
#   - M3 V6 (selector_m3.py): S1+S2+S3+S4, gate rescue 0.545, top-K=10.
#   - piso rescue 0.545 y modelo bge-m3 NO se tocan (regla de la serie).
#   - calibración post-hoc prohibida: PAS_PAD se barre {0,1,2,4} a priori.
#   - G2: --repeat 2 → dos corridas por pad; el JSON de cada corrida lleva
#     determinism_hash (excluye solo elapsed_seconds).
#
# Mecanismo de pool (idéntico a run-evidence-PC.sh EXPAND=f2, salvo PAS_PAD y
# dict H1):
#   L   lexicales  : buffy-search.sh OR top-50 + gate co-ocurrencia ≥2 tokens
#   X   expansión  : buffy-search.sh OR top-50 por término H1 real
#   S   semánticos : bge-m3 coseno top-50 (índice de líneas de D)
#   P   tiles      : ventanas no-solapadas de 2·PAS_PAD+1 líneas sobre
#                    F1 (router_knowledge) + F2 (top-K del pool por orden R1),
#                    tope max-passages=400 (kno entran completos, pool se recorta)
#
# Métricas (harness 15B): attr (needle en ctx Y en pasaje de gold_files),
# pRel, cross_domain_leakage, gold_containment. Baseline de 16B = PAS_PAD=4 del
# MISMO runner (el 11/20 del pool congelado ±4 NO es comparable — pool distinto).
#
# Uso:
#   run-granularity-PC.sh [--pads "0 1 2 4"] [--repeat 1|2] [--out <json>]
#                         [--repo <dir>] [--reindex] [--limit 10]
#     --pads     PAS_PAD a barrer (default: "0 1 2 4")
#     --repeat   corridas por pad para determinismo G2 (default: 1; 2 = G2)
#     --out      archivo JSON de resultados (una corrida)
#     --repo     repo con el corpus (default: $HOME/buffy-context)
#     --reindex  forzar rebuild del índice semántico de líneas (default: cache)
...
OUT_FILE="${OUT_FILE:-$SCRIPT_DIR/granularity-16B.json}"
PADS="0 1 2 4"
REPEAT="1"
LIMIT="10"
REINDEX="false"
...
repo, eval_path, eval_hash, out_file, pads_s, limit_s, cache_dir, reindex = (
...
H1_DICT_HASH = expand_query.dict_hash()
...
t0_all = time.time()
results = {"benchmark": "16B granularidad del pasaje (pool regenerado)",
