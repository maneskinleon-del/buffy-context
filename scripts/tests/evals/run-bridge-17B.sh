#!/usr/bin/env bash
# run-bridge-17B.sh — Paso 17B: puente semántico DICT_H1 (A/B) sobre Q01/Q05
# ─────────────────────────────────────────────────────────────────────────────
# Pregunta (H17, spec bridge-17B-DESIGN.md): con la MISMA config congelada de
# 16B (PAS_PAD=4, M3 V6, piso 0.545), ¿una variante ACOTADA de DICT_H1 que
# cubre el tramo LN → concepto técnico → comando/símbolo cruza el piso en
# Q01/Q05 sin regresión en prosa ni leak?
#
# Diseño: un solo A/B, una sola variable (DICT_H1):
#   A (control) = expand_query.DICT_H1 actual (sin --dict)
#   B (tratamiento) = --dict <json> con la variante congelada (dict_h1_b.json)
# Todo lo demás IDÉNTICO a 16B: EVAL, corpus congelado, PAS_PAD=4, M3 V6,
# LIMIT=10, piso 0.545, bge-m3, pool L∪X∪S∪P-F2, métricas 15B. Sin oráculo,
# sin inyección de gold. Anti-oráculo: gold_facts ∩ términos_nuevos = ∅.
#
# Uso:
#   run-bridge-17B.sh [--dict <json>] [--repeat 1|2] [--out <json>]
#                     [--repo <dir>] [--reindex] [--limit 10]
#     --dict     JSON de variante DICT_H1 (default: expand_query.DICT_H1 = A)
